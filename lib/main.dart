...
Future<void> main() async {
  ...
  try {
    ...
    // Plugin not implemented on this platform (e.g., desktop without plugin support)
    // ignore: avoid_print
    ...
    return MaterialApp(
      ...
      routes: {
        '/live':
            (ctx) =>
                cameras.isNotEmpty
                    ? LiveFeedPage(camera: cameras.first)
                    : Scaffold(
                      appBar: AppBar(title: const Text('Live Feed')),
                      body: const Center(
                        child: Text('No camera available on this platform'),
                      ),
                    ),
      },
    );
